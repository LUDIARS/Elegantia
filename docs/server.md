...
公開Webは基準・テンプレートの単語検索と閲覧だけを提供します。
以下の試験結果・申し送りの操作は、明示的なローカルモードで利用します。
設定は[実行環境](../spec/setup/runtime.md)を参照してください。

ローカル画面はCcの登録済みプロジェクトを更新順10件ずつ表示します。
保存済み評価条件を選択して評価を確認し、各項目に人間のOK/NGとコメントを追記できます。
調査分析はローカル環境のLLMで実行します。サーバから実行はできません。

公開URLに Cloudflare Access を掛け、その団体ドメインと Audience タグを `ELEGANTIA_CF_ACCESS_TEAM_DOMAIN` / `ELEGANTIA_CF_ACCESS_AUD` で渡すと、Access を通った利用者だけが評価結果・試験履歴・申し送りを閲覧できます (読み取りのみ)。設定が無ければ公開URLからは基準の検索だけです。

- 220項目を一覧にまとめ、5部構成の本文を横断検索。分野・試験状態で絞り込む。
...
調査分析（ローカルLLMによる対象プロジェクトの解析）の結果JSONとレポートは、本体の `.local/analysis/<プロジェクト名>/<日付>/` に置きます。`.local/` はGit管理対象外なので、privateプロジェクトの名称や仕様を含む結果を誤って公開リポジトリへ含めません。結果JSONは `shared/results.ts` のインポート形式に合わせ、Webのインポートで試験履歴へ取り込めます。
