...
export function HumanReview({ project, itemId, onSaved, canWrite }: { project: string; itemId: string; onSaved: () => void; canWrite: boolean }) {
  ...
  return <section><h3>人間の申し送り</h3><p>最新: {reviews[0]?.verdict ?? '未記録'}。試験の合否とは別に記録します。</p>
    {canWrite ? <form onSubmit={save}><label>判断<select value={verdict} onChange={e => setVerdict(e.target.value as 'OK' | 'NG')}><option>OK</option><option>NG</option></select></label>
      <label>人間のコメント<textarea required maxLength={12000} value={comment} onChange={e => setComment(e.target.value)} /></label>
      <button disabled={busy}>{busy ? '保存中…' : '判断とコメントを保存'}</button></form> : <p>閲覧のみです。申し送りの追加はローカルツールから行います。</p>}<p role="status">{message}</p>
